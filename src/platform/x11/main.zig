    ...
    switch (event_type) {
        ...
        xcb.XCB_KEY_PRESS => {
            ...
            const changed = switch (input.kind) {
                ...
                .paragraph => try editor_state.insertLineBreak(),
                .move_left => try editor_state.moveLeft(caret_map),
                .move_right => try editor_state.moveRight(caret_map),
                .move_up => try editor_state.moveVertical(caret_map, .up),
                .move_down => try editor_state.moveVertical(caret_map, .down),
                .delete_backward => try editor_state.backspace(caret_map),
